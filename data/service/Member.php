...
use data\model\AgentRechargeModel;
...
class Member extends User implements IMember
{
    ...
    public function getAgentLevel(){
        ...
        $res = parent::getUserInfo();
        $constant12 = $constant->getAConstant(12);
        $constant14 = $constant->getAConstant(14);
        $constant16 = $constant->getAConstant(16);
        switch ($res[agent]) {
            case '0':
                $percent = 1;
                break;
            case '1':
                $percent = $constant12[val]*0.1;
                break;
            case '2':
                $percent = $constant14[val]*0.1;
                break;
            case '3':
                $percent = $constant16[val]*0.1;
                break;
    ...
    public function getAgentMoney($level){
        $constant = new Constant();
        $constant11 = $constant->getAConstant(11);
        $constant13 = $constant->getAConstant(13);
        $constant15 = $constant->getAConstant(15);
        switch ($level) {
            case '0':
                $money = 0;
                break;
            case '1':
                $money = $constant11[val];
                break;
            case '2':
                $money = $constant13[val];
                break;
            case '3':
                $money = $constant15[val];
                break;
    ...
    public function getAccount(){
        ...
        $arr['point'] = $member_account->getMemberPoint($this->uid, '');
        $arr['balance'] = $member_account->getMemberBalance($this->uid, '');
        return $arr;
    }
    ...
    public function getMemberLevel(){
        ...
        $where['uid'] = $this->uid; 
        $res = $member->getUserMember($where);
        return $res;
    }
    ...
    public function getMemberById($uid){
        ...
        $res = $member->getInfo('uid=' . $uid,'*');
        $level = $member_level->getInfo([
            'level_id' => $res['member_level']
        ], 'goods_discount');
        return $level;
    }
    public function getMemberByTel($tuijian){
        $res=parent::getbytel($tuijian);
        return $res;

    }

    public function getAUser($tuijian){
        $user = new UserModel();
        $where['user_tel'] = $tuijian; 
        return $user->getInfo($where);
     ...
     */
    public function registerMember($user_name, $password, $email, $mobile, $user_qq_id, $qq_info, $wx_openid, $wx_info, $wx_unionid, $tuijian='', $real_name)
    {
        // if (! empty($user_name)) {
        ...
        $res = parent::add($user_name, $password, $email, $mobile, $tuijian, $real_name, 0, $user_qq_id, $qq_info, $wx_openid, $wx_info, $wx_unionid, 1);
        if ($res > 0) {
            // 获取默认会员等级id
            $member_level = new NsMemberLevelModel();
     ...
     */
    public function addMember($user_name, $password, $email, $sex, $status, $mobile, $member_level, $tuijian='', $real_name='')
    {
        $res = parent::add($user_name, $password, $email, $mobile, $tuijian, $real_name, 0, '', '', '', '', '', 1);
        if ($res > 0) {
            $member = new NsMemberModel();
            $data = array(
                'uid' => $res,
     ...
     * 通过id修改user激活状态
     */
    public function updateUserByUid($uid, $jihuo)
    {
        $user = new UserModel();
        $result = $user->save([
            'jihuo' => $jihuo
    ...
    public function getMemberList($page_index = 1, $page_size = 0, $condition = '', $order = '', $field = '*')
    {
        ...
        foreach ($result['data'] as $k => $v) {
			...
			if($v['par_tel']){
				$res = parent::getUserInfoByUid($v['par_tel']);
				if($res['real_name']){
					$result['data'][$k]['tuijian'] = $res['real_name'];
				}else{
					$result['data'][$k]['tuijian'] = $res['user_name'];
				}
			}
    ...
    public function getMemberTeamList($page_index = 1, $page_size = 0, $condition = '', $order = '', $field = '*')
    {
        $futou = new Futou();
        $member_view = new NsMemberViewModel();
        $result = $member_view->getViewList($page_index, $page_size, $condition, $order);
        foreach ($result['data'] as $k => $v) {
            ...
            $result['data'][$k]['futou'] = $futou->getInfoById($v['uid']);
            $result['data'][$k]['coin'] = $member_account->getMemberCoin($v['uid']);

            //推荐奖
            $conditions['nmar.shop_id'] = 0;
            $conditions['nmar.uid'] = $v['uid'];
            $conditions['nmar.account_type'] = 1;
            $conditions['nmar.from_type'] = 24;
            $member_point_list = $this->getAccountList(1, 0, $conditions);
            foreach ($member_point_list[data] as $key) {
               $point_sum += $key[number];
            }
            $result['data'][$k]['tuijianjiang'] = $point_sum;
            //团队返利
            $team['nmar.shop_id'] = 0;
            $team['nmar.uid'] = $v['uid'];
            $team['nmar.account_type'] = 1;
            $team['nmar.from_type'] = 27;
            $teamfen = $this->getAccountList(1, 0, $team);
            foreach ($teamfen[data] as $key) {
               $team_sum += $key[number];
            }
            $result['data'][$k]['team_sum'] = $team_sum;
            //日返利
            $rifen['nmar.shop_id'] = 0;
            $rifen['nmar.uid'] = $v['uid'];
            $rifen['nmar.account_type'] = 1;
            $rifen['nmar.from_type'] = 26;
            $accout26_list = $this->getAccountList(1, 0, $rifen);
            foreach ($accout26_list[data] as $key) {
               $accout26_sum += $key[number];
            }
            $result['data'][$k]['rifenhong'] = $accout26_sum;
            //团队人数
            $son = $this->getSon($v['uid']);
            $son =count($son);
            $sonTwo = $this->getSonTwo($v['uid']);
            $sonTwo =count($sonTwo);
            $sonThree = $this->getSonThree($v['uid']);
            $sonThree =count($sonThree);
            $team_num = $son + $sonTwo + $sonThree;
            $result['data'][$k]['team_num'] = $team_num;
     ...
     * mymy修改积分和余额
     */
    public function updateAccountData($shop_id, $type, $uid, $num, $from_type, $text)
    {
        $member_account = new MemberAccount();
        $retval = $member_account->addMemberAccountData($shop_id, $type, $uid, 1, $num, $from_type, 0, $text);
        return $retval;
    }


    public function kaihu_zhuan($shop_id, $type, $uid, $num, $text, $zhuan_uid,$handle_obj)
    {
        $member_account = new MemberAccount();
        // 应该用事务回滚
        $retval = $member_account->addMemberAccountData($shop_id, $type, $uid, 1, -$num, 21, 0, $text, $handle_obj[1]);
        if ($retval) {
            $res = $member_account->addMemberAccountData($shop_id, $type, $zhuan_uid, 1, $num, 22, 0, $text, $handle_obj[0]);
            return $res;
        }else{
            $retval = $member_account->addMemberAccountData($shop_id, $type, $uid, 1, $num, 23, 0, $text);
        }
    ...
    public function createMemberRecharge($recharge_money, $uid, $out_trade_no)
    {
        ...
        if ($res) {
            $percent = $this->getAgentLevel();
            $recharge_money = $recharge_money*$percent;
            $pay = new UnifyPay();
     ...
     * mymy创建代理订单
     */
    public function createAgent($money, $uid, $out_trade_no, $level)
    {
        $agent = new AgentRechargeModel();
        $data = array(
            'money' => $money,
            'uid' => $uid,
            'out_trade_no' => $out_trade_no,
        ...
        if ($res) {
            $pay = new UnifyPay();
            $pay->createPayment(0, $out_trade_no, '升级代理', '升级代理', $money, 9, $agent->id);
        }
        return $res;
    }
     ...
     * mymy支付成功回调代理升级
     */
    public function UpdateAgent($out_trade_no){
        $member_account = new MemberAccount();
        $agent = new AgentRechargeModel();
        $res = $agent->getInfo('out_trade_no='.$out_trade_no);
        $arr = array(
            "is_pay" => 1,
            "status" => 1   
        );
        $agent->save($arr, ["out_trade_no" => $out_trade_no]);
		
        $data = array(
             'agent' => $res[level]
         );
        
        $rr = parent::updateUserByUserid($res[uid],$data);
		$info = parent::getUserInfoByUid($res[uid]);
        if($info[par_tel]>0){
            $parent = parent::getUserInfoByUid($info[par_tel]);

            $award = $this->getAgentMoney($parent[agent]);
            $award = $award*$parent[agent]*0.1;
            
            $member_account->addMemberAccountData(0, 2, $info[par_tel], 1, $award, 31, 0, '子升级代理');
            return $award;
        }
     ...
     */
    public function updateMemberByAdmin($uid, $user_name, $email, $sex, $status, $mobile, $nick_name, $member_level, $tuijian, $real_name)
    {
        $retval = parent::updateUserInfo($uid, $user_name, $email, $sex, $status, $mobile, $nick_name, $tuijian, $real_name);
        if ($retval < 0) {
            return $retval;
        } else {
            // 修改会员等级
