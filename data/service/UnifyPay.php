...
class UnifyPay extends BaseService implements IUnifyPay
{
        ...
        try{
            ...
            switch ( $pay_info['type']){
                ...
                case 9:
                    //代理升级
                    $member = new Member();
                    $member->updateAgent($out_trade_no);
                    break;
                default:
                    break;
            }
    ...
    public function wchatPay2($out_trade_no, $trade_type, $red_url)
    {
        $data = $this->getPayInfo($out_trade_no);
        if($data < 0)
        {
        ...
        $percent = $member -> getAgentLevel();
        //mymy 生成微信二维码
        $retval = $weixin_pay->setWeiXinPay($data['pay_body'], $data['pay_detail'], $data['pay_money']*100*$percent, $out_trade_no, $red_url, $trade_type, $openid, $product_id);
        return $retval;
