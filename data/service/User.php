...
class User extends BaseService implements IUser
{
    ...
    public function getSon($uid){
        ...
        foreach ($res as $key => $val) {
            $futou_level = $futou->getInfoById($val[uid]);
            $res[$key]['level'] = $futou_level;
        }
        return $res;
    ...
    public function getSonTwo($uid){
        ...
        foreach ($one as $key) {
           $where['par_tel'] = $key['uid']; 
           $where['jihuo'] = 1;
           $rr = $this->user->where($where)->select();
           if ($rr) {
                foreach ($rr as $aa) {
                   $res[] = $aa;
                }
     ...
     * mymy 获取三级子会员
     */
    public function getSonThree($uid){
        $futou = new Futou();
        $two = $this->getSonTwo($uid);
        $res = array();
        if (!$two) {
           return $res;
        }
     ...
     * mymy修改个人信息
     */
     public function xiugaiInfo($uid,$arr)
    {
      $res = $this->user->save($arr, ['uid' => $uid]);
      return $res;
    }
	...
	public function getbyUsertel($tel){
      $where['user_tel'] = $tel;
      $res= $this->user->getInfo($where, '*');
      if ($res) {
        return true;
     ...
     */
    public function add($user_name, $password, $email, $mobile, $tuijian, $real_name, $is_system, $qq_openid, $qq_info, $wx_openid, $wx_info,$wx_unionid, $is_member, $instance_id = 0)
    {
        if (! empty($user_name)) {
        ...
        // //更新父级会员的等级
        // if ($tuijian) {
        //     $this->updateUserLevel(0, $tuijian);
        //     $user_par = $this->user->getInfo(['uid' => $tuijian], '*');
        //     if ($user_par['par_tel']) {
        //         $tuijian2 = $user_par['par_tel'];
        //         $this->updateUserLevel(0, $tuijian2);
        //         $user_par2 = $this->user->getInfo(['uid' => $tuijian2], '*');
        //         if ($user_par2['par_tel']) {
        //             $tuijian3 = $user_par2['par_tel'];
        //             $this->updateUserLevel(0, $tuijian3);
        //         }
        //     }
        // }
        // //赠送父级积分 
    ...
    public function updateUserInfo($uid, $user_name, $email, $sex, $status, $mobile, $nick_name, $tuijian, $real_name)
    {
     ...
     * 修改二级密码
     */
    public function ModifyErji($uid, $old_password, $new_password)
    {
        $condition = array(
            'uid' => $uid,
            'erji_psw' => md5($old_password)
        );
        $res = $this->user->getInfo($condition, $field = "uid");
        if (! empty($res['uid'])) {
            $data = array(
                'erji_psw' => md5($new_password)
            );
            $res = $this->user->save($data, [
                'uid' => $uid
            ]);
     ...
     */
     public function updateUserLevel($shop_id, $user_id){
        ...
        $shop_server=new Shop();
        #得到会员的直推人数
        $first_son=$this->getSon($user_id);
        $first_son = count($first_son);
        #得到会员的三代人数
        $one = $this->getSon($user_id);
        $two = $this->getSonTwo($user_id);
        $three = $this->getSonThree($user_id);
        $sum = count($one) + count($two) + count($three);
        #得到会员的信息
        $member_obj=$member_model->get($user_id);
        if(!empty($member_obj)){
            $level_id=$member_obj["member_level"];
            $member_level_model=new NsMemberLevelModel();
            $level_list=$member_level_model->getQuery("(upgrade=3 and relation=1 and (min_integral<=".$first_son." or quota<=".$sum.")) or (upgrade=3 and relation=2 and min_integral<=".$first_son." and quota<=".$sum.") or (upgrade=1 and min_integral<=".$first_son." ) or (upgrade=2 and quota<=".$sum.")", "*", "goods_discount desc");
    ...
    public function shengxing($id){
        $member_model=new NsMemberModel();
        $res = $member_model->save(["member_level"=>$id], ["uid"=>$this->uid]);
        return $res;
    }
