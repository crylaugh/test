...
class Order extends BaseService implements IOrder
{
     ...
     */
    public function orderCreate($order_type, $out_trade_no, $pay_type, $shipping_type, $order_from, $buyer_ip, $buyer_message, $buyer_invoice, $shipping_time, $receiver_mobile, $receiver_province, $receiver_city, $receiver_district, $receiver_address, $receiver_zip, $receiver_name, $point, $jifen, $coupon_id, $user_money, $goods_sku_list, $platform_money, $pick_up_id, $shipping_company_id, $coin = 0)
    {
        $order = new OrderBusiness();
        if ($pay_type == 4) {
        ...
        $retval = $order->orderCreate($order_type, $out_trade_no, $pay_type, $shipping_type, $order_from, $buyer_ip, $buyer_message, $buyer_invoice, $shipping_time, $receiver_mobile, $receiver_province, $receiver_city, $receiver_district, $receiver_address, $receiver_zip, $receiver_name, $point, $jifen, $coupon_id, $user_money, $goods_sku_list, $platform_money, $pick_up_id, $shipping_company_id, $coin);
            ...
            if ($retval > 0) {
                ...
                foreach ($order_list as $k => $v) {
                    ...
                    foreach ($order_item_list as $key=>$val) {
                        if ($val['goods_id'] == 3) {
                            $member =new Member();
                            $res = $member->updateUserByUid($this->uid,1);

                            //mymy 复投 并为直属父级加积分。
                            $futou = new Futou();
                            $res= $futou->addFuTou($val['buyer_id']);
                            //+
                            $user = new UserModel();
                            $user_info = $user->getInfo(['uid' => $this->uid]);
                            //获取常量 推荐积分2
                           if ($user_info['par_tel']) {
                                $reee = $account -> getAllToday($user_info['par_tel']);
                                if ($reee) {
                                    $constant = $constant->getAConstant(2);
                                    if ($constant['val'] > $reee) {
                                        $constant['val'] = $reee;
                                    }
									$member_account=new MemberAccount();
                                    $member_account->addMemberAccountData(0, 1, $user_info['par_tel'], 1, $constant['val'], 24, 0, '推荐得分');
                                }
                                
                            }

                            // //更新父级会员的等级
                            // $tuijian = $user_info['par_tel'];
                            // if ($tuijian) {
                            //     $member->updateUserLevel(0, $tuijian);
                            //     $user_par = $user->getInfo(['uid' => $tuijian, 'jihuo' => 1], '*');
                            //     if ($user_par['par_tel']) {
                            //         $tuijian2 = $user_par['par_tel'];
                            //         $member->updateUserLevel(0, $tuijian2);
                            //         $user_par2 = $user->getInfo(['uid' => $tuijian2, 'jihuo' => 1], '*');
                            //         if ($user_par2['par_tel']) {
                            //             $tuijian3 = $user_par2['par_tel'];
                            //             $member->updateUserLevel(0, $tuijian3);
                            //         }
                            //     }
                            // }
    ...
    public function orderGoodsConfirmRefund($order_id, $order_goods_id, $refund_real_money)
    {
        ...
        if($retval){
            ...
            $shuju = $order_goods->orderGoodsRefundBalance($order_goods_id);
            $res = $this->getOrderInfo($order_id);
            $data['tuikuan'] = $res['tuikuan']+$shuju[balance];
            $orderModel->save($data,['order_id' => $order_id]);
            // 积分返还
            $account_flow = new MemberAccount();
            if ($shuju[point] > 0) {
                $account_flow->addMemberAccountData($res['shop_id'], 1, $res['buyer_id'], 1, $shuju[point], 2, $orderid, '退货返还积分');
            }
            ...
            if ($shuju[balance] > 0) {
                $account_flow->addMemberAccountData(0, 2, $res['buyer_id'], 1, $shuju[balance], 2, $orderid, '退货返还平台余额');
            }
            

            // 退回库存
            $order_goodsModel = new NsOrderGoodsModel();
            $order_goods_list = $order_goodsModel->getInfo([
                'order_goods_id' => $order_goods_id
            ], '*', '');
                ...
                $goods_sku_info = $goods_sku_model->getInfo([
                    'sku_id' => $order_goods_list['sku_id']
                ], 'goods_id, stock');
                if ($order_goods_list['shipping_status'] != 1) {
                    // 卖家未发货
                    $return_stock = 1;
                } else {
                    // 卖家已发货,买家不退货
                    if ($order_goods_list['refund_type'] == 1) {
                        $return_stock = 0;
                    } else {
                        $return_stock = 1;
                    }
                ...
                if ($return_stock == 1) {
                    $data_goods_sku = array(
                        'stock' => $goods_sku_info['stock'] + $order_goods_list['num']
                    );
...
//             }
     ...
     * mymy一定时间段内订单的消费积分
     */
    public function getPayJifenSum($condition)
    {
        $order_model = new NsOrderModel();
        $jifen_sum = $order_model->where($condition)->sum('jifen');
        return $jifen_sum;
    }
    ...
    /**
     * mymy获取一定时间内购买激活大礼包的数量
     */
    public function getJihuoNumSum($condition)
    {
        $order_model = new NsOrderModel();
        $order_list = $order_model->where($condition)
            ->select();
        ...
        foreach ($order_list as $k => $v) {
            $order_goods = new NsOrderGoodsModel();
            $goods_sum += $order_goods->where([
                'order_id' => $v['order_id'], 'goods_id' => 3
            ])->sum('num');
        }
        return $goods_sum;
    }
    ...
    public function getOrderGoodsExpressMessage($express_id)
    {
        try {
            ...
            if (! empty($express_obj)) {
                ...
                $order_time_arr = $order_model->getInfo(['order_id'=>$express_obj["order_id"]],"create_time,pay_time,consign_time");
                $retval = array();
                if(!empty($order_time_arr['create_time'])){
                    $retval[0] = array("AcceptTime"=>date("Y-m-d H:i:s",$order_time_arr['create_time']),"AcceptStation"=>"您的订单已提交，请尽快完成支付");
                    if(!empty($order_time_arr['pay_time'])){
                        $retval[1] = array("AcceptTime"=>date("Y-m-d H:i:s",$order_time_arr['pay_time']),"AcceptStation"=>"您的订单已支付完成，请等待卖家发货");
                        if(!empty($order_time_arr['consign_time'])){
                            $retval[2] = array("AcceptTime"=>date("Y-m-d H:i:s",$order_time_arr['consign_time']),"AcceptStation"=>"您的订单已发货");
                        }
                    }
                }
     ...
     * @param unknown $operator_type  操作人类型  1商家  2用户
     * @param unknown $operator_id  操作人id
     */
    public function deleteOrder($order_id,$operator_type,$operator_id){
        ...
        if($operator_type == 1){
            //商家删除
            $res = $order_model->save($data,["order_id"=>["in",$order_id_array], "shop_id"=>$operator_id]);

        }elseif($operator_type == 2){
            //用户删除
            $res = $order_model->save($data,["order_id"=>["in",$order_id_array], "buyer_id"=>$operator_id]);
        }
        return 1;
    }
}