...
class Goods extends BaseService implements IGoods
{
    ...
    //mymy通过商品id 用户id查找是否有该订单
    public function findOrder($goods_id,$uid,$state=0)
    {
        $order_goods = new NsOrderGoodsModel();
        $where['goods_id'] = $goods_id;
        $where['buyer_id'] = $uid;
        $res = $order_goods->where($where)->select();
        if ($res) {
            $order = new NsOrderModel();
            foreach ($res as $key) {
                $arr['order_id'] = $key['order_id'];
                $arr['is_deleted'] = $state;
                $aa = $order->where($arr)->select();
                if ($aa) {
                    $rr = 1;
                }else{
                    $rr = 0;
     ...
     */
    public function addOrEditGoods($goods_id, $goods_name, $shopid, $category_id, $category_id_1, $category_id_2, $category_id_3, $supplier_id, $brand_id, $group_id_array, $goods_type, $market_price, $price,$jifen,$cost_price, $point_exchange_type, $point_exchange, $give_point, $is_member_discount, $shipping_fee, $shipping_fee_id, $stock, $max_buy, $min_buy, $min_stock_alarm, $clicks, $sales, $collects, $star, $evaluates, $shares, $province_id, $city_id, $picture, $keywords, $introduction, $description, $QRcode, $code, $is_stock_visible, $is_hot, $is_recommend, $is_new, $sort, $image_array, $sku_array, $state, $sku_img_array, $goods_attribute_id, $goods_attribute, $goods_spec_format, $goods_weight, $goods_volume, $shipping_fee_type, $extend_category_id, $sku_picture_values)
    {
        $error = 0;
    ...
    public function getCartList($carts)
    {
        ...
        foreach ($cart_list as $k => $v) {
            ...
            $goods_info = $goods->getInfo([
                'goods_id' => $v['goods_id']
            ], 'max_buy,state,point_exchange_type,point_exchange,jifen');
            // 获取商品sku信息
            $goods_sku = new NsGoodsSkuModel();
            $sku_info = $goods_sku->getInfo([
                'sku_id' => $v['sku_id']
     ...
     */
    public function addCart($uid, $shop_id, $shop_name, $goods_id, $goods_name, $sku_id, $sku_name, $price, $num, $picture, $bl_id ,$jifen)
    {
        // 检测当前购物车中是否存在产品
        if ($uid > 0) {
            $cart = new NsCartModel();
