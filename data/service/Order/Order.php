...
class Order extends BaseService
{
    ...
    public function orderCreate($order_type, $out_trade_no, $pay_type, $shipping_type, $order_from, $buyer_ip, $buyer_message, $buyer_invoice, $shipping_time, $receiver_mobile, $receiver_province, $receiver_city, $receiver_district, $receiver_address, $receiver_zip, $receiver_name, $point, $jifen, $coupon_id, $user_money, $goods_sku_list, $platform_money, $pick_up_id, $shipping_company_id, $coin)
    {
        ...
        try {
            ...
            $res_order_goods = $order_goods->addOrderGoods($order_id, $goods_sku_list,0,$jifen);
        ...
        if (! empty($order_obj)) {
            ...
            if (empty($order_no_max)) {
                ...
                if (substr($time_str, 0, 8) == substr($order_no_max, 0, 8)) {
                    $max_no = substr($order_no_max, 14, 7);
     ...
     * @return Exception
     */
    public function orderClose($orderid,$flag = 1)
    {
        $this->order->startTrans();
        try {
            $data_close = array(
            ...
            $order_info = $this->order->getInfo([
                'order_id' => $orderid
            ], 'pay_status,jifen, coupon_id, user_money, buyer_id,shop_id,user_platform_money, coin_money');
            if ($flag) {
            
                // 积分返还
                $account_flow = new MemberAccount();
                if ($order_info['jifen'] > 0) {
                    $account_flow->addMemberAccountData($order_info['shop_id'], 1, $order_info['buyer_id'], 1, $order_info['jifen'], 2, $orderid, '订单关闭返还积分');
                }
    ...
    public function orderGoodsRefundFinish($order_id)
    {
        ...
        try {
            ...
            if ($all_refund == 0) {
                ...
                // 全部退款订单转化为交易关闭
                $retval = $this->orderClose($order_id,0);
            }
    ...
    public function orderAdjustMoney($order_id, $goods_money, $shipping_fee)
    {
        ...
        try {
            $order_model = new NsOrderModel();
            $order_info = $order_model->getInfo(['order_id' => $order_id], 'goods_money,shipping_money,order_money,pay_money');
            // 商品金额差额
