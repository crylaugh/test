...
class OrderGoods extends BaseService
{
 ...
 // 订单主表
     ...
     */
    public function addOrderGoods($order_id, $goods_sku_list, $adjust_money = 0,$jifen)
    {
        ...
        try {
            ...
            foreach ($goods_sku_list_array as $k => $goods_sku_array) {
                ...
                ], 'goods_name,price,goods_type,picture,promotion_type,promote_id,point_exchange_type,give_point,jifen');

                if ($goods_sku[1] * $goods_info['jifen'] >= $jifen) {
                    $xy_jifen = $jifen;
                    $jifen = 0;
                }else{
                    $xy_jifen = $goods_sku[1] * $goods_info['jifen'];
                    $jifen -= $goods_sku[1] * $goods_info['jifen'];
                }
    ...
    public function orderGoodsConfirmRefund($order_id, $order_goods_id, $refund_real_money)
    {
        ...
        try {
            ...
            if ($order_info["give_point_type"] == 3 && $order_info["give_point"] > 0) {
    ...
    public function orderGoodsRefundMoney($order_goods_id)
    {
        ...
        $order_goods_info = $order_goods->getInfo([
            'order_goods_id' => $order_goods_id
        ], 'order_id,sku_id,goods_money,jifen');
        ...
        $money = $order_goods_info['goods_money'] - $promotion_money-$order_goods_info['jifen'];
        // 计算其他方式支付金额
        ...
        $all_other_pay_money = $order_other_pay_money['user_money'] + $order_other_pay_money['coin_money'] + $order_other_pay_money['user_platform_money']-$order_other_pay_money['tax_money'];
        if ($all_other_pay_money != 0) {
            $proportion =  $money / ($order_other_pay_money['order_money']-$order_other_pay_money['shipping_money']-$order_other_pay_money['tax_money']);
            $other_pay = $proportion*$all_other_pay_money;
    ...
    public function orderGoodsRefundBalance($order_goods_id)
    {
        ...
        $balance = $order_goods_info['goods_money'] - $promotion_money - $order_goods_info['jifen'];
        // 计算其他方式支付金额
        $order = new NsOrderModel();
        $order_other_pay_money = $order->getInfo([
            'order_id' => $order_goods_info['order_id']
        ...
        $all_other_pay_money = $order_other_pay_money['user_money'] + $order_other_pay_money['coin_money'] + $order_other_pay_money['user_platform_money']-$order_other_pay_money['tax_money'];
        if ($all_other_pay_money != 0) {
            $proportion =  $balance / ($order_other_pay_money['order_money']-$order_other_pay_money['shipping_money']-$order_other_pay_money['tax_money']);
            $balance = round($order_other_pay_money['user_platform_money']*$proportion,2);
        }
       
        if ($balance < 0) {
            $balance = 0;
        }
        $arr['point'] = $order_goods_info['jifen'];
        $arr['balance'] = $balance;
        return $arr;
    }
