...
class Login extends Controller
{
    ...
    public function addUser()
    {
        ...
        if (empty($user_name)) {
            ...
            $result = $this->user->registerMember($user_name, $password, $tuijian, $real_name, $email, $mobile, $qq_openid, $qq_info, $wx_openid, $wx_info);
            if ($result > 0) {
    ...
    public function register()
    {
        if (request()->isAjax()) {
            ...
            $real_name = request()->post('real_name','');
            $sendMobile = Session::get('sendMobile');
            $tuijianren =$member->getAUser($tuijian);
            $tuijian = $tuijianren['uid'];
            if (empty($mobile)) {
                $retval = $member->registerMember($user_name, $password, $email, $mobile, '', '', '', '', '', $tuijian, $real_name);
            } else {
     ...
     public function getuser_tel()
    {
        if (request()->isAjax()) {
            $tuijian = request()->post('tuijian','');
            $member = new Member();
            $exist = $member->getMemberByTel($tuijian);
            return $exist;
        }
    }
