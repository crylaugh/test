...
function init(){
	...
	$("div[data-jifen]").each(function(){
		//循环小计
		total_jifen += parseFloat($(this).attr('data-jifen'));
	})
	point=$("div[user_point]").attr('user_point');
	
	if (point>total_jifen) {
		$(".keyong_jifen").text(total_jifen);
		$("#jfdyje").val(total_jifen);
	}else{
		$(".keyong_jifen").text(point);
		$("#jfdyje").val(point);
	}

	var jinfen=$("#jfdyje").val();
...
function validationOrder(){
	...
	//mymy 判断是否是新人大礼包  如果是 将不能在线支付
	if ($("#hidden_new_people").val()==1) {
		if (parseFloat($("#realprice").attr("data-total-money")) != 0) {
			showBox("激活礼包只能用余额支付。");
			return false;
		}
	}
	if($("#hidden_fixed_point").val() < $("#account_balance").val()){
		showBox("提交出错");
		return false;
	}
...
function calculateTotalAmount(){
	...
	var jifen=$("#jilu_jifen").text();
	if ($("#sfjfdy").prop("checked")) {
		$("#jfdyje").val(parseFloat(jifen));
		money -= parseFloat(jifen);
		tax_sum -= parseFloat(jifen);
	}else{
		$("#jfdyje").val(0);
	}
