...
$(function(){
	...
	$(".btn-jiesuan").click(function(){
		if(validationOrder()){
			...
			$.ajax({
				url : __URL(shop_main + "/order/ordercreate"),
				type : "post",
				data : {
				...
				success : function(res) {
					if (res.code > 0) {
						...
						if(parseFloat($("#realprice").attr("data-total-money")) == 0){
							location.href = __URL(app_main + '/pay/paycallback?msg=1&out_trade_no=' + res.code);
						}else if(pay_type == 4){
							location.href = __URL(shop_main + '/member/orderlist');
						}else{
							window.location.href = __URL(app_main + '/pay/getpayvalue?out_trade_no=' + res.code);
						}
					}else{
						$.msg(res.message);
						flag = false;
