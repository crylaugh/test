<?php if (!defined('THINK_PATH')) exit(); /*a:10:{s:54:"template/adminblue\Goods\selectCategoryNextUpdate.html";i:1507442312;s:28:"template/adminblue\base.html";i:1508840920;s:45:"template/adminblue\controlCommonVariable.html";i:1503719297;s:61:"template/adminblue\Goods\controlEditGoodsCommonResources.html";i:1505468035;s:32:"template/adminblue\urlModel.html";i:1504865854;s:42:"template/adminblue\Goods\fileAlbumImg.html";i:1503654581;s:56:"template/adminblue\Goods\controlEditGoodsCommonHTML.html";i:1500522318;s:58:"template/adminblue\Goods\controlEditGoodsCommonScript.html";i:1505468625;s:34:"template/adminblue\pageCommon.html";i:1505550036;s:34:"template/adminblue\openDialog.html";i:1509005619;}*/ ?>
<!DOCTYPE html>
<html>
	<head>
...
.brand_list .brand:hover,.brand_list .selected{
	background: #1E90FF;
	color: #fff;
	cursor: pointer;
}
.brand_list .search_brand{
	width: 100%;
	height: 30px;
	border-bottom: 1px solid #ccc;
}
.brand_list .search_brand input{
	width: 180px;
	border: none;
	float: right;
	padding-left: 0;
}
.brand_list .search_brand input:focus{
	box-shadow: none;
}
.brand_list .search_brand i.icon-search-tabao{
	width: 30px;
	height: 30px;
	display: inline-block;
	float: left;
...
#brand_list{max-height: 300px;overflow: overlay;}
</style>
<script>
...
<!-- 						<?php if($brand['brand_id'] == $goods_info['brand_id']): ?> -->
...
<!-- 		</dd> -->
			<dd>
				<div style="position:relative;" id="selectBrand">
				<div class="select_brand" data-brand-id="0">请选择商品品牌</div>
				<div class="brand_list">
					<div class="search_brand">
						<i class="icon-search-tabao"></i>
						<input type="text" placeholder="请输入品牌名称" id="search_brand" onkeyup="select_brand_list();">
					</div>
					<div id="brand_list">
						<div class="brand" data-brand-id="0">请选择商品品牌</div>
						<?php if(is_array($goodsbrand_list) || $goodsbrand_list instanceof \think\Collection || $goodsbrand_list instanceof \think\Paginator): if( count($goodsbrand_list)==0 ) : echo "" ;else: foreach($goodsbrand_list as $key=>$brand): ?>
						<div class="brand <?php if($brand['brand_id'] == $goods_info['brand_id']): ?>selected<?php endif; ?>" data-brand-id="<?php echo $brand['brand_id']; ?>"><?php echo $brand['brand_name']; ?></div>
						<?php endforeach; endif; else: echo "" ;endif; ?>
					</div>
				</div>
				<input name="box" id="bank_id" style="width:110px;position:absolute;left:7px;top:8px;" readonly="readonly" placeholder="请选择商品品牌">
 			</div>
			</dd>
	</dl>
	<dl>
	...
	<dl>
		...
		<dd>
			<div class="controls">
				<input type="number" id="jifendixian" min="0" placeholder="0" value="<?php echo $goods_info['jifen']; ?>" style="width:114px;" />
...
function file_upload(obj){
	...
	<h3 class="js-goods-point" style="display: none">积分购买设置</h3>
...
// 		}
	...
	//选择商品品牌
	$(function(){
		var selectBrandName = $("#brand_list .selected").text();
		var selectBrandId = $("#brand_list .selected").attr("data-brand-id");
		$(".select_brand").text(selectBrandName);
		$("#bank_id").val(selectBrandName);
		$(".select_brand").attr("data-brand-id",selectBrandId);
	})
	var is_show_brand_list = 0
	$("#selectBrand").click(function(){
		if(is_show_brand_list == 0){
			$(".brand_list").show();
			is_show_brand_list = 1;
		}else{
			$(".brand_list").hide();
			is_show_brand_list = 0;
		}
	})
	//查找商品品牌
	function select_brand_list(){
		var search_brand = $("#search_brand").val();
		$.ajax({
			type : "post",
			url : __URL('ADMIN_MAIN/goods/goodsBrandList'),
			data : { "search_text": search_brand },
			success : function(data){
				var html = '';
				if(data["data"].length > 0){
					html += '<div class="brand" data-brand-id="0">请选择商品品牌</div>';
					for (var i = 0; i < data["data"].length; i++) {
						var item = data["data"][i];
						html += '<div class="brand" data-brand-id="'+item["brand_id"]+'">'+item['brand_name']+'</div>';
					}
				}
	...
	$("#brand_list .brand").live("click",function(){
		$("#brand_list .brand").removeClass("selected");
		$(this).addClass("selected");
		var brand_name = $(this).text();
		var brand_id = $(this).attr("data-brand-id");
		$(".select_brand").text(brand_name);
		$("#bank_id").val(brand_name);
		$(".select_brand").attr("data-brand-id",brand_id);
		$(".brand_list").hide();
	})
</script>
