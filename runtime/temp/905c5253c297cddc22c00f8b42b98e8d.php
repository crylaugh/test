<?php if (!defined('THINK_PATH')) exit(); /*a:6:{s:44:"template/adminblue\Cms\articleClassList.html";i:1501559752;s:28:"template/adminblue\base.html";i:1505297354;s:45:"template/adminblue\controlCommonVariable.html";i:1503719297;s:32:"template/adminblue\urlModel.html";i:1504865854;s:34:"template/adminblue\pageCommon.html";i:1505550036;s:34:"template/adminblue\openDialog.html";i:1500263975;}*/ ?>
<!DOCTYPE html>
<html>
	<head>
...
<li><a href="javascript:;" onclick="addUpdateArticleClassShow(0);"><i class="fa fa-plus-circle"></i>&nbsp;添加</a></li>
	...
	<table class="table-class">
		...
		<tbody>
			...
			<tr class="pid_0" style="height: 30px;">
				<td>
					<?php if($v1 != array()): ?>
					<a href="javascript:;" onclick="tab_switch(<?php echo $v1['class_id']; ?>)" class="tab_jia_<?php echo $v1['class_id']; ?>" style="display: block;"><i class="fa fa-plus-circle"></i></a>
					<a href="javascript:;" onclick="tab_switch(<?php echo $v1['class_id']; ?>)" class="tab_jian_<?php echo $v1['class_id']; ?>" style="display: none;"><i class="fa fa-minus-circle"></i></a>
					<?php endif; ?>
				</td>
				<td>
					<input type="number" class="input-common" min="0" onkeyup="value=value.replace(/[^\d+(\.\d+)?]/g,'')" fieldid="<?php echo $v1['class_id']; ?>" fieldname="sort" value="<?php echo $v1['sort']; ?>" style="width:50px;">
				</td>
				<td><input type="text" class="input-common" fieldid="<?php echo $v1['class_id']; ?>" fieldname="name" value="<?php echo $v1['name']; ?>"></td>
				<td style="text-align: center;">
					<a href="javascript:void(0);" onclick="addUpdateArticleClassShow(<?php echo $v1['class_id']; ?>);">修改</a>
					<a href="javascript:void(0);" onclick="deleteArticleClass(<?php echo $v1['class_id']; ?>)">删除</a>
				</td>
			</tr>
			...
			<tr class="pid_<?php echo $v1['class_id']; ?>" style="height: 30px; display: none;">
				<td></td>
				<td>
					<input type="number" class="input-common" min="0" onkeyup="value=value.replace(/[^\d+(\.\d+)?]/g,'')" fieldid="<?php echo $v2['class_id']; ?>" fieldname="sort" value="<?php echo $v2['sort']; ?>" style="width:50px;">
				</td>
				<td>
					<span style="color: #ccc;">|——</span>
					<input type="text" class="input-common" fieldid="<?php echo $v2['class_id']; ?>" fieldname="name" value="<?php echo $v2['name']; ?>">
				</td>
				<td style="text-align: center;">
					<a href="javascript:void(0);" onclick="addUpdateArticleClassShow(<?php echo $v2['class_id']; ?>);">修改</a>
					<a href="javascript:void(0);" onclick="deleteArticleClass(<?php echo $v2['class_id']; ?>)">删除</a>
				</td>
			</tr>
				...
				<div class="control-group" style="display:none;">
					...
					<div class="controls">
						<select id="pid" name="account">
							...
							<option value="<?php echo $vo['class_id']; ?>" pid="<?php echo $vo['pid']; ?>">&nbsp;&nbsp;&nbsp;<?php echo $vo['name']; ?></option>
							<?php endforeach; endif; else: echo "" ;endif; ?>
						</select>
					</div>
				</div>
...
function addUpdateArticleClassShow(class_id){
	...
	if(class_id>0){
		...
		$.ajax({
			type:'post',
			url:"<?php echo __URL('ADMIN_MAIN/cms/articleclassinfo'); ?>",
...
function save(){
	...
	$.ajax({
		type:'post',
		url:"<?php echo __URL('ADMIN_MAIN/cms/addupdatearitcleclass'); ?>",
		data:{
			'class_id':class_id,
			'name':name,
			'sort':sort,
	...
	$( "#dialog" ).dialog({
		buttons: {
			"确定": function() {
				...
				$.ajax({
					type : "post",
					url : "<?php echo __URL('ADMIN_MAIN/cms/deletearticleclass'); ?>",
					dataType : "json",
					data : { "class_id" : class_id.toString() },
					success : function(data) {
						if (data["code"] > 0) {
							showMessage('success', data["message"], "<?php echo __URL('ADMIN_MAIN/cms/articleclasslist'); ?>");
						}else{
...
function load(event){
	...
	$.ajax({
		type:"post",
		url:"<?php echo __URL('ADMIN_MAIN/Cms/modifyField'); ?>",
...
$(".table-class input").change(function(){
	...
	$.ajax({
		type:"post",
		url:"<?php echo __URL('ADMIN_MAIN/cms/cmsfield'); ?>",
