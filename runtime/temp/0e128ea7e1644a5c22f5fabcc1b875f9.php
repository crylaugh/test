<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:42:"template/wap\default\\Member\erji_psw.html";i:1507866113;s:30:"template/wap\default\base.html";i:1507430043;s:34:"template/wap\default\urlModel.html";i:1505377008;}*/ ?>
<!DOCTYPE html>
<html>
<head>
...
<form class="mt-55 mlr-15" id="editpassword" style="background-color:#fff;margin:45px 0 0 0" >
		<div><span>二级密码：</span>
			<input type="text" id="erji_psw" style="margin:0;height:50px;border-bottom:none;"  placeholder="请输入二级密码"   onfocus="$(this).attr('type','password')">
		</div>
		
		<div><span>确认二级密码：</span>
			<input type="text" id="erji_psw2" style="box-shadow:none;margin:0;height:35px;border:none;width:auto;display:inline-block;"  placeholder="确认密码"  onfocus="$(this).attr('type','password')"/>
...
<script type="text/javascript">
	...
	function btnsave() {
        ...
        var value = "";
        //密码(6-16)位
        var erji_psw = $("#erji_psw").val();
        var erji_psw2 = $("#erji_psw2").val();
        
        var reg = /^[\@A-Za-z0-9\!\#\$\%\^\&\*\.\~]{6,16}$/;
         if (erji_psw.length == 0) {
        	showBox("<?php echo lang('please_enter_the_original_password'); ?>", {
				time: 2000
			});
            return false;
        }
        if (!reg.test(erji_psw)) {
        	showBox("<?php echo lang('please_enter_6_20_new_passwords'); ?>", {
				time: 2000
			});
            return false;
        }
        if(erji_psw!=erji_psw2){
        ...
        $.ajax({
            url: "<?php echo __URL('APP_MAIN/member/erji_psw'); ?>",
            data: { "erji_psw":erji_psw},
            type: "post",
