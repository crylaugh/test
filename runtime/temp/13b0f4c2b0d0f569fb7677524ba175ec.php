<?php if (!defined('THINK_PATH')) exit(); /*a:6:{s:41:"template/adminblue\Member\memberList.html";i:1509691306;s:28:"template/adminblue\base.html";i:1508840920;s:45:"template/adminblue\controlCommonVariable.html";i:1503719297;s:32:"template/adminblue\urlModel.html";i:1504865854;s:34:"template/adminblue\pageCommon.html";i:1505550036;s:34:"template/adminblue\openDialog.html";i:1509005619;}*/ ?>
<!DOCTYPE html>
<html>
	<head>
			...
			<div class="modal-body">
				<div class="modal-infp-style">
					<table class="modal-tab">
						<tr>
							<td style="width:20%;"><span class="required">*</span>手机号</td>
							<td colspan='3'>
								<input type="text" id="user_tel" class="input-common" />
						...
						<tr>
							<td style="width:22%;"><span class="required">*</span>姓名</td>
							<td colspan='3'>
								<input type="text" id="real_name" class="input-common" />
							</td>
						</tr>
						
						<tr>
							<td style="width:22%;">推荐人</td>
							<td colspan='3'>
								<input type="text" id="tuijian" class="input-common" />
							</td>
						</tr>
						<tr>
							<td>会员等级</td>
				...
				<div class="modal-infp-style">
					<table class="modal-tab">
						<tr>
							<td style="width:20%;"><span class="required">*</span>手机号</td>
							<td colspan='3'>
								<input type="text" id="modify_user_tel" class="input-common" />
								<span id="usernameyz"></span>
								<input type="hidden" value="不存在" id="isset_username" class="input-common" />
							</td>
						</tr>
						<tr>
							<td style="width:22%;"><span class="required">*</span>姓名</td>
							<td colspan='3'>
								<input type="text" id="modify_real_name" class="input-common" />
							</td>
						</tr>
						...
						<tr>
							<td style="width:22%;">推荐人</td>
							<td colspan='3'>
								<input type="text" id="modify_tuijian" class="input-common" />
							</td>
						</tr>
						<tr>
							<td>会员等级</td>
...
// 								html += '昵称 : '+data["data"][i]["nick_name"] +'<br/>'+'手机 : '+data["data"][i]["user_tel"]+'<br/>'+'推荐人 : '+data["data"][i]["tuijian"];
					...
					html += '</td>';
						if (data["data"][i]["agent"]==0) {
							html += '<td class="tal">--</td>';
						}else{
							html += '<td class="tal">'+data["data"][i]["agent"]+'级</td>';
						}
...
$("#user_tel").blur(function(){
	...
	if(tel === $("#modify_username_hidden").val()){
		return;
	}
	checkUserTel(tel);
});

function checkUserTel(user_tel){
	var flag = true;
	$.ajax({
		type: "GET",
		url: "<?php echo __URL('ADMIN_MAIN/member/check_user_tel'); ?>",
		async : false,
		...
		success: function(data){
			if(data){
				...
				$("#usernameyz").css("color","red").text("手机号已存在");
...
function checkTuiJian(tuijian){
	var flag = true;
	$.ajax({
		type: "GET",
		url: "<?php echo __URL('ADMIN_MAIN/member/checkTuiJian'); ?>",
		async : false,
...
function addUser(){
	...
	if (real_name == '') {
		showTip('姓名不能为空','warning');
		return;
	}
	if (user_tel == '') {
		showTip('手机号不能为空','warning');
		return;
	}
	
	if(!checkUserTel(user_tel)){
		showTip('手机号已存在','warning');
		return;
	}
	...
	if(user_tel.length > 0){
		if(!(/^1(3|4|5|7|8)\d{9}$/.test(user_tel))){ 
	...
	if(tuijian.length > 0){
		if(!(/^1(3|4|5|7|8)\d{9}$/.test(user_tel))){ 
			showTip("推荐人手机号有误",'warning');
			return; 
		}
		if (!checkTuiJian(tuijian)) {
			showTip("请输入正确的推荐人",'warning');
			return; 
		}
...
function modify_user(uid){
	...
	$.ajax({
		...
		success : function(data) {
			...
			if(data['user_tel']!=''){
				$("#modify_user_tel").val(data.user_tel);
				
			}
			
			$("#modify_real_name").val(data.real_name);
			//$("#modify_password").val(data.user_password);
			$("#modify_username_hidden").val(data.user_tel);
			$("#modify_tuijian").val(data.par_tel);
...
function modifyUser(){
	...
	var real_name = $("#modify_real_name").val();
	var tuijian = $("#modify_tuijian").val();
