...
<div id="pagination" class="page" >
	...
	<div class="page-num fr">
		...
		<div id="page_list" style="float: left;<?php echo $path_info; ?>">
			...
			<!-- 总页数小于总页码时就都显示 -->
			<?php if($page_count <= $page_num): $__FOR_START_2821__=1;$__FOR_END_2821__=$page_count+1;for($i=$__FOR_START_2821__;$i < $__FOR_END_2821__;$i+=1){ if($i == $page): ?>
			...
			<?php elseif($page <= ($page_num-1)/2): $__FOR_START_31375__=1;$__FOR_END_31375__=$page_num+1;for($i=$__FOR_START_31375__;$i < $__FOR_END_31375__;$i+=1){ if($i == $page): ?>
						<span class="num curr"><a href="javascript:;" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php else: ?>
						<span class="num"><a href="<?php echo __URL('SHOP_MAIN/'.$path_info.'?page='.$i.$query_string); ?>" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php endif; } ?>
			<!-- 如果总页数等于当前页 或者 总页数小于当前页加上页码总数平均值显示 -->
			<?php elseif($page_count == $page or $page_count <= $page+($page_num-1)/2): $__FOR_START_19224__=$page_count-$page_num+1;$__FOR_END_19224__=$page_count+1;for($i=$__FOR_START_19224__;$i < $__FOR_END_19224__;$i+=1){ if($i == $page): ?>
						<span class="num curr"><a href="javascript:;" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php else: ?>
						<span class="num"><a href="<?php echo __URL('SHOP_MAIN/'.$path_info.'?page='.$i.$query_string); ?>" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php endif; } ?>
			<!-- 否则就正常显示 -->
			<?php else: $__FOR_START_22156__=$page-($page_num-1)/2;$__FOR_END_22156__=$page+($page_num-1)/2+1;for($i=$__FOR_START_22156__;$i < $__FOR_END_22156__;$i+=1){ if($i == $page): ?>
						<span class="num curr"><a href="javascript:;" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php else: ?>
						<span class="num"><a href="<?php echo __URL('SHOP_MAIN/'.$path_info.'?page='.$i.$query_string); ?>" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php endif; } endif; ?>
					...
					<div class="modal-infp-style">
						<table>
							<tr>
								...
								<td colspan='3'>
									<input type="text" placeholder="<?php echo lang('member_input_balance'); ?>" id="pay_money" style="height: 25px;"/>
								</td>
							</tr>
						</table>
...
function calculateRecharge(){
	...
	if(pay_money == ''){
		$.msg("<?php echo lang('member_amount_can_not_empty'); ?>");
		return false;
	}
	if(pay_money < 0){
		$("#pay_money").val(0);
