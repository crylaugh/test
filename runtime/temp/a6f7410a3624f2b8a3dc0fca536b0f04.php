<?php if (!defined('THINK_PATH')) exit(); /*a:6:{s:41:"template/adminblue\System\editModule.html";i:1503280183;s:28:"template/adminblue\base.html";i:1508840920;s:45:"template/adminblue\controlCommonVariable.html";i:1503719297;s:32:"template/adminblue\urlModel.html";i:1504865854;s:34:"template/adminblue\pageCommon.html";i:1505550036;s:34:"template/adminblue\openDialog.html";i:1509005619;}*/ ?>
<!DOCTYPE html>
<html>
	<head>
...
<div class="set-style">
	...
	<dl>
		<dt>模块名:</dt>
		<dd>
			<p><input id="module_name" type="text" value="<?php echo $module_info['module_name']; ?>" class="input-common"></p>
		...
		<dd>
			<p>
				<select class="form-control input-sm select-common" id="pid">
					...
					<?php if(!(empty($list) || (($list instanceof \think\Collection || $list instanceof \think\Paginator ) && $list->isEmpty()))): if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): if( count($list)==0 ) : echo "" ;else: foreach($list as $key=>$v1): ?>
							<option value="<?php echo $v1['data']['module_id']; ?>" <?php if($module_info['pid'] == $v1['data']['module_id']): ?>selected<?php endif; ?>>&nbsp;&nbsp;&nbsp;<?php echo $v1['data']['module_name']; ?></option>
								<?php if(!(empty($v1['sub_menu']) || (($v1['sub_menu'] instanceof \think\Collection || $v1['sub_menu'] instanceof \think\Paginator ) && $v1['sub_menu']->isEmpty()))): if(is_array($v1['sub_menu']) || $v1['sub_menu'] instanceof \think\Collection || $v1['sub_menu'] instanceof \think\Paginator): if( count($v1['sub_menu'])==0 ) : echo "" ;else: foreach($v1['sub_menu'] as $key=>$v2): ?>
	...
	<dl>
		<dt>控制器名:</dt>
		<dd>
			<p><input id="controller" type="text" value="<?php echo $module_info['controller']; ?>" class="input-common"></p>
	...
	<dl>
		<dt>方法名:</dt>
		<dd>
			<p><input id="method" type="text" value="<?php echo $module_info['method']; ?>" class="input-common"></p>
	...
	<dl>
		<dt>URL:</dt>
		<dd>
			<p><input id="url" type="text" value="<?php echo $module_info['url']; ?>" class="input-common"></p>
	...
	<dl>
		<dt>排序:</dt>
		<dd>
			<p><input id="sort" type="text" value="<?php echo $module_info['sort']; ?>" class="input-common"></p>
	...
	<dl>
		...
		<dd>
			<div class="class-logo">
				...
				<img id="imglogo" src="<?php echo __IMG($module_info['module_picture']); ?>" >
				<?php else: ?>
				<img id="imglogo">
				<?php endif; ?>
				</p>
	...
	<dl>
		<dt>描述:</dt>
		<dd>
			<p><textarea id="desc" class="input-common"><?php echo $module_info['desc']; ?></textarea></p>
...
function editModuleAjax() {
	...
	if(verify(module_name, controller, method, url, sort, desc)){
		...
		$.ajax({
			type : "post",
			url : "<?php echo __URL('ADMIN_MAIN/system/editmodule'); ?>",
			data : {
				'module_id' : module_id,
				'pid' : pid,
			...
			success : function(data) {
				if (data["code"] > 0) {
					showMessage('success', data["message"],"<?php echo __URL('ADMIN_MAIN/system/modulelist'); ?>");
				}else{
