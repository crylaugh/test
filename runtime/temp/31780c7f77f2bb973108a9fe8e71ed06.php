<?php if (!defined('THINK_PATH')) exit(); /*a:6:{s:39:"template/adminblue\Goods\goodsList.html";i:1505464789;s:28:"template/adminblue\base.html";i:1508840920;s:45:"template/adminblue\controlCommonVariable.html";i:1503719297;s:32:"template/adminblue\urlModel.html";i:1504865854;s:34:"template/adminblue\pageCommon.html";i:1505550036;s:34:"template/adminblue\openDialog.html";i:1509005619;}*/ ?>
<!DOCTYPE html>
<html>
	<head>
		...
		success : function(data) {
			...
			if (data["data"].length > 0) {
				for (var i = 0; i < data["data"].length; i++) {
					...
					html += '<div class="QRcode" style="display: none; position: absolute;width:110px;top:28px;left:0;"id="qrcode"><p><img src="'+ __IMG(data["data"][i]["QRcode"])+'" style="width:110px;"></p></div></span>';
					html += '</span></div></td></tr>';
					html += '<tr><td colspan="2" style="background: white;"><div><a class="a-pro-view-img" href="'+__URL('SHOP_MAIN/goods/goodsinfo?goodsid='+data["data"][i]["goods_id"])+'" target="_blank" style="height:70px;line-height:70px;text-align:center;"><img class="thumbnail-img"src="'+__IMG(data["data"][i]["pic_cover_micro"])+'"></a>';
					html += '<div class="div-pro-view-name"><span style="color: #13A5D5;" class="thumbnail-name title='+ data["data"][i]["goods_name"]+'"><div class="editGoodsIntroduction padding_none" ondblclick="editGoodsInfo(this)"><a target="_blank" style="word-break:break-all;" href="'+__URL('SHOP_MAIN/goods/goodsinfo?goodsid='+data["data"][i]["goods_id"])+'">'
						+ data["data"][i]["goods_name"]
						+ '</a></div><input class="js-update-goods-name" data-goods-id ="' + data['data'][i]['goods_id'] + '" data-up-type="goods_name" type="text" value="'+data["data"][i]['goods_name']+'"/></span><br/>';
					if(data["data"][i]['introduction'] != '' && data["data"][i]['introduction'] !=undefined){
						html += '<div class="editGoodsIntroduction" ondblclick="editGoodsInfo(this)"><span style="color:#999;font-size:12px;display:block;height:25px;overflow:hidden;text-decoration: none;">'+data["data"][i]['introduction']+'</span></div>';
						html += '<input  data-goods-id ="' + data['data'][i]['goods_id'] + '" data-up-type="introduction" class="js-update-introduction" type="text" maxlength="60" value="'+data["data"][i]['introduction']+'" /><br>';
					}
					if(data["data"][i]['goods_group_name'] != '' && data["data"][i]['goods_group_name'] != undefined){
						...
						$.each(tmp_array,function(k,v){			
							html += '<span style="color:#999;font-size:12px;margin-top:5px;padding:2px 4px;border-radius:3px;display:inline-block;color:#FFF;background-color:#de533c;width:40px;text-decoration: none;height:16px;line-height: 16px;overflow:hidden;margin-right:5px;text-align:center;">'+v+'</span>';
				...
				//	html += '</div>';
					...
					html += '<td style="background: white;"><div class="cell"><input class="input-mini" goods_id="'
						+ data["data"][i]["goods_id"]
						+ '" style="width:38px;text-align:center;" value="'
						+ data["data"][i]["sort"]
					...
					html += '<a href="' + __URL("ADMIN_MAIN/goods/addgoods?step=2&goodsId="+ data["data"][i]["goods_id"]) + '" data-placement="bottom" data-original-title="编辑"><span class="edit" style="display: inline-block; width: 19%;"><i class="icon-edit" style="width: initial;"></i>编辑</span></a>';
